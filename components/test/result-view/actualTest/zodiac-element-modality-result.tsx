...
import { 
  Flame, Mountain, Wind, Droplets, Zap, Target, RefreshCw, 
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-white px-0 sm:px-4">
      ...
      >
        ...
        <div className="flex flex-col px-[32px] pt-6 pb-12 flex-1 overflow-y-auto">
          {/* Header Section */}
          <div className="mb-6">
            <h1 className="text-[26px] font-[500] text-[#FFFFFF] leading-tight mb-2">
              {data.title || "The Energetic Soul"}
            </h1>
            <div className="flex items-center gap-2 text-[13px] font-[400] tracking-wider" style={{ color: accentColor }}>
          ...
          {/* Energy Profile (2 paragraphs) */}
          <div className="mb-8 p-6 bg-white/5 border border-white/10 rounded-2xl">
            <h3 className="text-white/40 text-[10px] uppercase tracking-[2px] mb-4 flex items-center gap-2">
               <BookOpen className="w-3 h-3" /> Energy Profile
            </h3>
            <div className="space-y-4">
              {data.energyProfile?.split('\n\n').map((paragraph, i) => (
                <p key={i} className="text-white/90 text-[14px] leading-relaxed font-[300]">
                  {paragraph}
                </p>
              )) || (
          ...
          <div className="mb-10">
            <h2 className="text-white/40 text-[11px] uppercase tracking-[2px] font-[500] mb-4">Core Traits</h2>
            <div className="grid grid-cols-1 gap-3">
              {(data.coreTraits || []).slice(0, 3).map((trait: string, i: number) => (
                <div key={i} className="flex items-center gap-3 bg-white/[0.03] p-3 rounded-xl border border-white/5">
                    ...
                    <span className="text-white/90 text-[13px] font-[300]">{trait}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 mb-10">
            <div className="bg-[#A2F2CD]/5 border border-[#A2F2CD]/10 p-4 rounded-2xl">
                <h2 className="text-[#A2F2CD] text-[10px] uppercase tracking-[2px] font-[500] mb-3">Strengths</h2>
                <div className="space-y-2">
                    {(data.strengths || []).slice(0, 3).map((s: string, i: number) => (
                        <p key={i} className="text-white/90 text-[12px] font-[400] leading-tight">• {s}</p>
                    ))}
                </div>
            </div>
            <div className="bg-[#F2A2A2]/5 border border-[#F2A2A2]/10 p-4 rounded-2xl">
                <h2 className="text-[#F2A2A2] text-[10px] uppercase tracking-[2px] font-[500] mb-3">Challenges</h2>
                <div className="space-y-2">
                    {(data.challenges || []).slice(0, 3).map((c: string, i: number) => (
                        <p key={i} className="text-white/70 text-[12px] font-[300] leading-tight">• {c}</p>
                    ))}
                </div>
            </div>
          </div>
          ...
          {data.shadowPattern && (
            <div className="mb-10 p-5 bg-white/[0.02] border-t border-b border-white/10">
               <h2 className="text-white/40 text-[11px] uppercase tracking-[2px] font-[500] mb-3 flex items-center gap-2">
                  <ShieldAlert className="w-4 h-4 opacity-50" /> Shadow Pattern
               </h2>
          ...
          {(data.dailyEvolution || []).length > 0 && (
            <div className="mb-12">
               <h2 className="text-white/40 text-[11px] uppercase tracking-[2px] font-[500] mb-4 flex items-center gap-2">
                  <Sparkles className="w-4 h-4 text-amber-200/50" /> Daily Evolution
               </h2>
               <div className="space-y-4">
                  {(data.dailyEvolution || []).slice(0, 2).map((tip: string, i: number) => (
                    <div key={i} className="relative pl-6">
                        <div className="absolute left-0 top-1.5 w-2 h-2 rounded-full border border-amber-200/30" />
                        <p className="text-white/90 text-[13px] font-[300] leading-relaxed">{tip}</p>
                    </div>
                  ))}
               </div>
            </div>
          ...
          <div className="mt-auto pt-8 border-t border-white/5 flex justify-around opacity-30 pb-4">
             <div className="flex flex-col items-center gap-1">
                ...
                <span className="text-[9px] uppercase tracking-wider">{element}</span>
             </div>
             <div className="flex flex-col items-center gap-1">
                {MODALITY_ICONS[modality]}
                <span className="text-[9px] uppercase tracking-wider">{modality}</span>
             </div>
          </div>
        </div>
      </div>
...
function SmallTraitCard({ label, value }: { label: string; value: string }) {
  return (
    <div className="bg-[#FFFFFF0D] border border-white/5 rounded-xl py-3 px-2 flex flex-col items-center justify-center">
      <span className="text-[9px] uppercase tracking-[1px] text-white/40 mb-1">{label}</span>
      <span className="text-[12px] font-[400] text-white/90 truncate w-full text-center">{value}</span>
    </div>
  );
}
