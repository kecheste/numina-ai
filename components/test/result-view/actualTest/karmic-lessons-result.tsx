...
export function KarmicLessonsResult({
  ...
  const dimensions = [
    { key: "boundaries", label: "Boundaries", color: "#F2D08C" },
    { key: "voice", label: "Voice", color: "#BA8CF2" },
    { key: "trust", label: "Trust", color: "#8CCBF2" },
    { key: "worthiness", label: "Worthiness", color: "#8CF2BC" },
    { key: "control", label: "Surrender", color: "#F28C8C" },
    { key: "patience", label: "Divine Timing", color: "#F2BC8C" },
  ];
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-white px-0 sm:px-4">
      ...
      >
        ...
        <div className="flex flex-col px-[32px] pt-6 pb-12 flex-1 overflow-y-auto">
          ...
          <h2 className="text-[13px] font-[300] text-[#BA8CF2] mb-4">
            Your Result
          </h2>

          {extracted.recurrence_score !== undefined && (
            <div className="mb-8 bg-white/5 p-4 rounded-xl border border-white/10 flex flex-col items-center">
              ...
              <span className="text-[32px] text-[#BA8CF2] font-bold">
                {extracted.recurrence_score}%
              </span>
              <div className="w-full h-1 bg-white/10 rounded-full mt-2 overflow-hidden">
                <div
                  className="h-full bg-[#BA8CF2]"
                  style={{ width: `${extracted.recurrence_score}%` }}
                />
              </div>
              <p className="text-[10px] text-white/30 mt-2 text-center">
                Strength of recurring karmic themes in your life
              </p>
            </div>
          )}
          ...
          {coreTraits.length > 0 && (
            <div className="mb-8">
              <h2 className="text-[#BA8CF2] font-semibold mb-3 text-[15px]">
                Core Traits
              </h2>
              <ul className="space-y-3">
                {coreTraits.map((trait: string, i: number) => (
                  <li key={i} className="flex items-start gap-3">
                    <div className="w-1.5 h-1.5 rounded-full bg-[#BA8CF2] mt-2 shrink-0" />
          ...
          {data.spiritualInsight && (
            <div className="mb-8 bg-white/5 p-5 rounded-xl border-l-4 border-[#BA8CF2]">
              <h2 className="text-[#BA8CF2] font-semibold mb-2 text-[15px]">
                Spiritual Insight
              </h2>
              <p className="text-white/90 text-[14px] leading-relaxed font-[250] italic">
                {data.spiritualInsight}
              </p>
            </div>
          )}
