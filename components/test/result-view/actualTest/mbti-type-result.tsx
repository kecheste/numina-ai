...
export function MbtiTypeResult({
  ...
  const typeLabel = result?.personality_type ?? result?.llm_result_json?.title ?? "Personality Type";
  const shortDescription = 
    result?.llm_result_json?.shortDescription || 
    result?.narrative?.split("\n").filter(Boolean)[0] || 
    "";
    
  const coreTraits = ensureStringArray(result?.llm_result_json?.coreTraits ?? result?.insights);
  const strengths = ensureStringArray(result?.llm_result_json?.strengths ?? result?.insights); 
  const challenges = ensureStringArray(result?.llm_result_json?.challenges ?? result?.recommendations);
  
  const narrative = result?.narrative || result?.llm_result_json?.summary;
  const confidence = result?.extracted_json?.confidence || result?.llm_result_json?.confidence;
