...
import { AstrologyChartResultView, NumerologyResultView } from "./result-view";
...
export function TestFlow({
  ...
  const [numerologyData, setNumerologyData] =
    useState<NumerologyResponse | null>(null);

  const isAstrologyChartTest = testId === 1;
  const isNumerologyTest = testId === 2;
  const isAutoGeneratedTest = isAstrologyChartTest || isNumerologyTest;
  ...
  useEffect(() => {
    ...
    } else if (isNumerologyTest) {
      apiFetchNumerology()
        .then((data) => {
          if (!cancelled) setNumerologyData(data);
        })
        .catch((e) => {
          if (!cancelled)
            setError(
              e instanceof Error ? e.message : "Failed to load numerology",
            );
        })
        .finally(() => {
          if (!cancelled) setLoading(false);
  ...
  }, [testId, isAstrologyChartTest, isNumerologyTest]);
  ...
  if (isNumerologyTest && numerologyData) {
    return (
