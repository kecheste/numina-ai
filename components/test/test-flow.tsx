...
export interface TestFlowProps {
  ...
  onClose: () => void;
  onboardingNext?: () => void;
  /** When true, test is auto-generated from profile (except Astrology 1); use questionless flow: submit → loading → result */
...
export function TestFlow({
  ...
  // All auto-generated tests (except Astrology 1) + Numerology 2, Transits 5, Zodiac 6: no questions, submit then show loading → result
  ...
  const MAX_POLL_ATTEMPTS = 24; // ~60 seconds then stop
  ...
  // For tests like Transits (5) and Zodiac Element & Modality (6) that currently have no questions,
  // automatically submit an empty answer payload so we can still generate an LLM-based result.
  useEffect(() => {
    if (
      !isAutoGeneratedTest &&
  ...
  if (showResults) {
    ...
    // Still waiting for the worker to complete – show beautiful preparing screen (polling will stop after ~60s).
    return (
